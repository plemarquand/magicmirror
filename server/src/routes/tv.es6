...
export default (url, apikey) => {
	...
	router.get('/tv', (req, res) => Rx.Observable.combineLatest(
            loadHistory(sb),
            loadLastSearchDate(dateFilename),
            (history, lastDate) => ({shows: history.data, lastDate})
        )
        .flatMap(({shows, lastDate}) => Rx.Observable.combineLatest(
            Rx.Observable.just(lastDate.toString()),
            filterDownloadedShows(shows, lastDate).map(getUniqueShows),
            (date, shows) => ({date, shows})
        ))
